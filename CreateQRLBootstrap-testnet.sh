...
NET_NAME=Testnet
QRL_DATA=/mnt/qrl/.qrl-testnet/data/state
BACKUP_PATH=/home/fr1t2/qrl-chain-state-backup/qrl_bootstrap_files
...
while [ $attempts -gt 0 ]; do
  ...
  copy_success=$?
  echo "["`date -u`"] Rsync exit code: $copy_success" | tee -a $BOOTSTRAP_LOGS
...
python3 $REALPATH/QRL_bootstrap/verify_leveldb.py $BACKUP_PATH/$NET_NAME/ | tee -a $BOOTSTRAP_LOGS
