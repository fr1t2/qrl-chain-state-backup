...
if [[ ! -f $QRL_DIR/$QRL_WALLET ]]; then
  ...
  wallet_success=$?
  echo "["`date -u`"] create-wallet exit code: $wallet_success" | tee -a $BOOTSTRAP_LOGS
...
# Get next OTS
#OTS_KEY=$(sudo -H -u $user qrl-cli ots $QRL_ADDRESS -g 127.0.0.1:29009 -j |grep next_key |jq .[0].next_key)
...
#echo sudo -H -u $user qrl-cli notarize $SHASUM -M "https://qrl.co.in/chain/#testnet-chain" -g 127.0.0.1:29009 -w $QRL_DIR/$QRL_WALLET -i $OTS_KEY -j
echo sudo -H -u $user qrl-cli notarize $SHASUM -M "https://qrl.co.in/chain/#testnet-chain" -t -w $QRL_DIR/$QRL_WALLET -i $OTS_KEY -j

#NOTARIZE=$(sudo -H -u $user qrl-cli notarize $SHASUM  -M "https://qrl.co.in/chain/#testnet-chain" -g 127.0.0.1:29009 -w $QRL_DIR/$QRL_WALLET -i $OTS_KEY -j )
NOTARIZE=$(sudo -H -u $user qrl-cli notarize $SHASUM  -M "https://qrl.co.in/chain/#testnet-chain" -t -w $QRL_DIR/$QRL_WALLET -i $OTS_KEY -j )
...
CHAIN_STATE=$(sudo -H -u $user /home/$user/.local/bin/qrl --json --port_pub 29009 state)
